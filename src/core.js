...
var Page = (function pagePage() {
  ...
  constructor.prototype = {
    ...
    display: function pageDisplay(gfx, callback) {
          ...
          for (var x = 0; x < allRows.length; x++) {
            allRows[x].onclick = (function(x) {
              return function() { 
                ...
                breakPoint = self.getNextBreakPoint();
              }
            })(x);
          }
      ...
      var pdfDebugger = new PdfDebugger();
      pdfDebugger.showPopup();
      /**** END HACK *****/

      var self = this;
      function next() {
        startIdx = gfx.executeIRQueue(IRQueue, startIdx, next, breakPoint);
        if (gfx.onBreakPoint) {
          pdfDebugger.breakIt(startIdx, gfx, next);
        }
        if (startIdx == length) {
